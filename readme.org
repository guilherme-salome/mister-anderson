...
Mr. Anderson is a FastAPI-based assistant that speeds up warehouse data entry. It lets operators upload product photos through the web dashboard, uses OpenAI Vision to extract key fields (serial number, description, subcategory, destination), and stores the results in SQLite alongside the processed images. A lightweight Flask web UI remains available for legacy product browsing.

Status: early development (alpha). I’m iterating with a warehouse user to make this truly useful in day-to-day work.

Features:
- FastAPI dashboard for managing pickups, pallets, and product entries
...
- src/
  ...
  - webapp/ — FastAPI application (routes, templates, static assets)
  - webui.py — legacy Flask app to list/view products and serve images
...
Typical flow:
1) Launch the FastAPI app and open the pallet detail view.
2) Upload photos through the “Add product” sheet; the vision model proposes populated fields.
3) Review/edit extracted fields, choose the correct destination/subcategory, then store the product.
4) Browse saved products from the dashboard or the legacy Flask UI at [[http://localhost:9090]].

* Setup

After cloning the repository and setting up Python with the listed packages in [[environment.yml]], configure the OpenAI credential in =~/.authinfo= (Emacs authinfo format):
#+begin_src conf
...
# Binds to 0.0.0.0:8000 (set PORT=8080 to override)
#+end_src

Optionally, you can launch the legacy Flask “products” browser in a separate terminal:
#+begin_src sh
python -m src.webui
...
On a browser:
1. Open [[http://localhost:9090]] to see recent products (ID, asset tag, pickup, qty, serial, subcategory, destination, description).
